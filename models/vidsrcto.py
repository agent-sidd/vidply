...
async def get_source(source_id:str,SOURCE_NAME:str) -> str:
    api_request:str = await fetch(f"https://vidsrc.xyz/ajax/embed/source/{source_id}")
...
async def get(dbid:str,s:int=None,e:int=None):
    ...
    id_url = f"https://vidsrc.xyz/embed/{media}/{dbid}" + (f"/{s}/{e}" if s and e else '')
    ...
    if id_request.status_code == 200:
        try:
            ...
            if sources_code == None:
                ...
                source_id_request = await fetch(f"https://vidsrc.xyz/ajax/embed/episode/{sources_code}/sources")
